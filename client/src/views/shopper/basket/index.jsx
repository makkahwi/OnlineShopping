import { cilMinus, cilPlus, cilTrash } from '@coreui/icons';
import CIcon from '@coreui/icons-react';
import { CButton, CButtonGroup, CImage, CTable, CTableBody, CTableDataCell, CTableFoot, CTableHeaderCell, CTableRow, CTooltip } from '@coreui/react';
import React, { useState } from 'react';
import Content from '../../../components/content';
import { useDispatch, useSelector } from 'react-redux'

export default function Basket() {

  const tempImage = "data:image/jpeg;base64,";

  const listings = [
    { id: 1, image: tempImage, name: "Product 1", desc: "Nice", price: 60, stock: 910, sold: 90 },
    { id: 2, image: tempImage, name: "Product 2", desc: "Good", price: 50, stock: 159, sold: 91 },
    { id: 3, image: tempImage, name: "Product 3", desc: "Good", price: 40, stock: 159, sold: 91 },
    { id: 4, image: tempImage, name: "Product 5", desc: "Good", price: 30, stock: 159, sold: 91 },
    { id: 5, image: tempImage, name: "Product 5", desc: "Good", price: 40, stock: 159, sold: 91 },
    { id: 6, image: tempImage, name: "Product 6", desc: "Good", price: 50, stock: 159, sold: 91 },
    { id: 7, image: tempImage, name: "Product 7", desc: "Good", price: 60, stock: 159, sold: 91 },
    { id: 8, image: tempImage, name: "Product 8", desc: "Good", price: 70, stock: 159, sold: 91 },
    { id: 9, image: tempImage, name: "Product 9", desc: "Good", price: 80, stock: 159, sold: 91 },
    { id: 10, image: tempImage, name: "Product 10", desc: "Good", price: 90, stock: 159, sold: 91 }
  ];

  const dispatch = useDispatch()
  const basket = useSelector((state) => state.basket)

  const [items, setItems] = useState(basket)

  const actionsClick = (i, action) => {
    let newList = [...items]
    newList[i]["count"] = (action === "add" ? newList[i]["count"] + 1 : newList[i]["count"] - 1)
    if (newList[i]["count"] === 0) {
      dispatch({ type: 'setBasket', basket: newList.filter((_, j) => j !== i) })
      setItems(newList.filter((_, j) => j !== i))
    }
    else {
      dispatch({ type: 'setBasket', basket: newList })
      setItems(newList)
    }
  };

  const onSubmit = () => {
    console.log("Ordering", items)
  };

  return (
    <Content
      title={"Items to Order"}
      content={
        <>
          <CTable hover>
            <CTableBody>
              {items.map((item, i) => {
                const listing = listings.find(it => it.id === item.id)

                return (
                  <CTableRow className='align-middle' key={i}>
                    <CTableDataCell>
                      <CImage src={listing.image} width={50} />
                    </CTableDataCell>

                    <CTableDataCell>{listing.name}</CTableDataCell>

                    <CTableDataCell>${listing.price}</CTableDataCell>

                    <CTableDataCell>${listing.price * item.count}</CTableDataCell>

                    <CTableDataCell>
                      <CButtonGroup size="sm">
                        <CTooltip
                          content="Add"
                          placement="top"
                        >
                          <CButton color="success" onClick={() => actionsClick(i, "add")}>
                            <CIcon className='text-light' icon={cilPlus} />
                          </CButton>
                        </CTooltip>

                        <CButton color="light">
                          {item.count}
                        </CButton>

                        <CTooltip
                          content="Reduce"
                          placement="top"
                        >
                          <CButton color="danger" onClick={() => actionsClick(i, "reduce")}>
                            <CIcon className='text-white' icon={item.count === 1 ? cilTrash : cilMinus} />
                          </CButton>
                        </CTooltip>
                      </CButtonGroup>
                    </CTableDataCell>
                  </CTableRow>
                )
              }
              )}
            </CTableBody>

            <CTableFoot>
              <CTableRow>
                <CTableHeaderCell></CTableHeaderCell>
                <CTableHeaderCell></CTableHeaderCell>
                <CTableHeaderCell></CTableHeaderCell>

                <CTableHeaderCell>
                  <h4>
                    {"$"}
                    {items.reduce((sum, order) => sum + (listings.find(listing => listing.id === order.id)?.price * order.count), 0)}
                  </h4>
                </CTableHeaderCell>

                <CTableHeaderCell>
                  <CButton color='success' size='lg' className='text-light' onClick={onSubmit}>
                    Make the order
                  </CButton>
                </CTableHeaderCell>
              </CTableRow>
            </CTableFoot>
          </CTable>

        </>
      }
    />
  )
}
