import { cilLowVision, cilNewspaper, cilPencil, cilTrash } from '@coreui/icons';
import CIcon from '@coreui/icons-react';
import {
  CButton, CButtonGroup, CFormInput, CImage, CInputGroup,
  CInputGroupText, CTable,
...
export default function Listings() {
  ...
  const listings = [
    { id: 1, image: tempImage, title: "Product 1", desc: "Nice", price: 60 },
    { id: 2, image: tempImage, title: "Product 2", desc: "Good", price: 50 }
  ];
  ...
  const modelContent = () => (<>
    {Object.keys(detailsData).map(key => key == "image" ? (
      ...
      <CInputGroup className="mb-3">
        <CInputGroupText className='text-capitalize'>{key}</CInputGroupText>
        <CFormInput value={detailsData[key]} disabled={action == "View"} />
      </CInputGroup>
    ))}
  </>);
    ...
    <Content
      ...
      content={
        <>
            ...
            <CTableBody>
              {listings.map(listing => (
                <CTableRow className='align-middle'>
                  ...
                  <CTableDataCell>{listing.title}</CTableDataCell>
          ...
          <Modal title={`Product ${action}`} body="Body" open={open} setOpen={setOpen} action={action} data={detailsData} content={modelContent()} onSubmit={() => onSubmit()} />
        </>
      }
    />
  )
